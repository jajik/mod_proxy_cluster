...
static char *process_config(request_rec *r, char **ptr, int *errtype)
{
    ...
    if (node != NULL) {
        ...
        if (!is_same_node(node, &nodeinfo)) {
            ...
            ap_log_error(APLOG_MARK, APLOG_ERR, 0, r->server, "process_config: node %s %d %s : %s %s already exists",
                         node->mess.jvm_route, node->mess.id, node->mess.port, nodeinfo.mess.jvm_route,
                         nodeinfo.mess.port);
            loc_unlock_nodes();
            *errtype = TYPEMEM;
            return apr_psprintf(r->pool, "MEM: Node with \"%s\" JVMRoute already exists", node->mess.jvm_route);
        }
    }
    ...
    if (id == -1) {
        ...
        if (id == -1 && balancerhandler != NULL) {
            ...
            ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server, "process_config: NEW (%s) %s://%s:%s in %d",
                         nodeinfo.mess.jvm_route, nodeinfo.mess.type, nodeinfo.mess.host, nodeinfo.mess.port, id);
        }
    }
    ...
    if (clean == 0) {
        ...
        ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server,
                     "process_config: (%s) (%s://%s:%s) inserted/updated in worker %d", nodeinfo.mess.jvm_route,
    ...
    for (i = 0; i < aliases->nelts; i++) {
        ...
        if (rv != APR_SUCCESS) {
            ...
            char *tmp =
                apr_psprintf(r->pool, MHOSTUI " (%s)", nodeinfo.mess.jvm_route, APR_ARRAY_IDX(aliases, i, char *));
        ...
        for (j = 0; j < contexts->nelts; j++) {
            ...
            if (rv != APR_SUCCESS) {
                /* TODO: Clean this as part of #330 */
                char *tmp = apr_psprintf(r->pool, MCONTUI " (%s and alias: %s)", nodeinfo.mess.jvm_route,
                                         APR_ARRAY_IDX(contexts, j, char *), APR_ARRAY_IDX(aliases, i, char *));
                err_msg = err_msg == NULL ? tmp : apr_pstrcat(r->pool, err_msg, ", ", tmp, NULL);
            }
        }
...
static char *process_app_cmd(request_rec *r, char **ptr, int cmd, int *errtype)
{
    ...
    for (j = 0; j < aliases->nelts; j++) {
        ...
        if (host == NULL) {
            ...
            ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server,
                         ...
                         nodeinfo.mess.jvm_route, current_alias);
    ...
    print_app_cmd_response(r, cmd, nodeinfo.mess.jvm_route, updated_aliases_contexts, aliases, contexts);
...
static char *process_status(request_rec *r, const char *const *ptr, int *errtype)
{
    ...
    print_status_response(r, nodeinfo.mess.jvm_route, isnode_up(r, node->mess.id, load) == OK);
    return NULL;
}
...
static char *process_ping(request_rec *r, const char *const *ptr, int *errtype)
{
    ...
    print_ping_response(r, nodeinfo.mess.jvm_route, node_up);
    return NULL;
}
...
static void print_hosts(request_rec *r, int reduce_display, int allow_cmd, int node, const char *jvm_route)
{
    ...
    id = apr_palloc(r->pool, sizeof(int) * size);
    size = get_ids_used_host(hoststatsmem, id);
    id_checker = apr_pcalloc(r->pool, sizeof(int) * size);
    for (i = 0; i < size; i++) {
        hostinfo_t *ou;
        if (get_host(hoststatsmem, &ou, id[i]) != APR_SUCCESS) {
            continue;
...
static void manager_hooks(apr_pool_t *p)
{
    ...
    ap_hook_translate_name(manager_trans, NULL, asz_succ, APR_HOOK_FIRST);
